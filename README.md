...
1. 选择共享文件夹后，在应用底部选择计费网络策略并启用远程下载。
2. 登录 `https://api.dongyanjun.xyz/admin/offline-download`。
3. 推送 HTTP 或 HTTPS 直链，并在页面查看进度、取消或重试任务。
4. 文件由手机直接从原始地址下载到共享文件夹，不经过远程服务器。
