...
- 远程控制页按设备显示当前网络名称、局域网访问地址、访问码以及电量和充电状态
- 电脑浏览器与安卓设备可通过 WebRTC 纯 P2P 直传文件，服务器只交换握手信令
- P2P 支持浏览共享目录、上传、下载、进度和取消，不配置 TURN 或文件中继
- 应用内检查、下载并安装更新，使用版本化缓存并校验哈希、包名、版本和签名
- Android 5 及更旧浏览器自动进入无脚本兼容页，可完成登录和文件管理
...
Android 5 及更旧的浏览器会自动进入兼容页面，也可以手动访问
`http://设备地址:端口/lite`。兼容页面不依赖 JavaScript，支持登录、目录浏览、
上传、下载、新建文件夹、重命名和删除。
...
进入“远程”页时，应用会申请精确位置权限，用于读取 Android 保护的
Wi-Fi 网络名称，不会读取定位坐标。拒绝权限不会影响局域网地址、访问码
或远程下载；系统热点名称受 Android 限制时会在后台明确标注。
...
- 网络名称、局域网地址和访问码通过设备令牌认证的 HTTPS 心跳上报，只在管理员后台显示。
...
app/src/main/java/com/lanfileserver/app/
  ...
  DeviceLanStatus.kt       网络名称和局域网访问信息上报

app/src/main/assets/web/
  login.html               浏览器登录页
  index.html               响应式文件管理器
  lite-*.html              Android 5 及更旧浏览器兼容页
```
